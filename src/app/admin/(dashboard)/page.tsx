...
import { Package, ShoppingBag, FolderTree, Percent, TrendingUp, Clock, AlertTriangle } from 'lucide-react';
...
export default async function AdminDashboardPage() {
  ...
  const totalRevenueResult = await adminSupabase.from('orders').select('total_amount').neq('status', 'cancelled');
  const totalRevenue = totalRevenueResult.data?.reduce((sum, o) => sum + (o.total_amount || 0), 0) || 0;
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-2xl p-6 border border-slate-200/80 shadow-xs hover:shadow-md transition-all space-y-2">
          ...
          <span className="text-[11px] text-slate-400 font-medium">From confirmed & active orders</span>
        </div>
        ...
        <div className="bg-white rounded-2xl p-6 border border-slate-200/80 shadow-xs hover:shadow-md transition-all space-y-2">
          ...
          <p className="text-3xl font-black text-slate-900 tracking-tight">{orderCount || 0}</p>
        ...
        <div className="bg-white rounded-2xl p-6 border border-slate-200/80 shadow-xs hover:shadow-md transition-all space-y-2">
          ...
          <p className="text-3xl font-black text-slate-900 tracking-tight">{productCount || 0}</p>
        ...
        <div className="bg-white rounded-2xl p-6 border border-slate-200/80 shadow-xs hover:shadow-md transition-all space-y-2">
          ...
          <p className="text-3xl font-black text-slate-900 tracking-tight">{settings?.global_discount_percentage || 0}% OFF</p>
